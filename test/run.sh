#!/usr/bin/env bash
# pi-safe slice-1 end-to-end + tamper tests.
# Builds a throwaway "real repo" and a separate "agent staging tree", then drives
# pi-safe through the gate for benign, failing, and tampering patches.
set -u
...
[ -f "$XDG_CONFIG_HOME/pi-safe/manifests"/*.json ] 2>/dev/null && ok "trusted manifest stored in harness-owned config dir" || bad "trusted manifest not in harness dir"
ls "$REAL"/.pi-safe* >/dev/null 2>&1 && bad "evaluator config leaked into real repo" || ok "no evaluator config in agent-writable repo"
...
if unshare -rnpf --mount-proc true 2>/dev/null; then
  ...
  MAN="$XDG_CONFIG_HOME/pi-safe/manifests/$RID.json"; mkdir -p "$(dirname "$MAN")"
